...
def train(
    ...
    for step in tqdm(range(n_step)):
        ...
        if step % 500 == 0:
            ...
            # save model
            if not scriptFolder.joinpath(f"./results/{D}dim").exists():
                scriptFolder.joinpath(f"./results/{D}dim").mkdir(parents=True)
            if save:
                torch.save(
                    model.state_dict(),
                    scriptFolder.joinpath(
                        f"./results/{D}dim/{exp_name}_checkpoint.pth.tar"
                    ),
                )
                if acc > best_acc:
                    best_acc = acc
                    shutil.copy(
                        ...
                        scriptFolder.joinpath(
                            f"./results/{D}dim/{exp_name}_best.pth.tar"
                        ),
                    )
    ...
    if key_mem_transform in (bipolarize, binarize):
        ...
        for i in tqdm(range(n_step)):
            ...
            with torch.no_grad():
                ...
                query_keys = model(query_set)
                cosine_sim = get_cosine_similarity(query_keys, support_keys)  # Cosine
                sharpened = sharpening_softabs(cosine_sim, 10)  # Cosine
                normalized = normalize(sharpened)
                if sum_argmax:
...
if __name__ == "__main__":
    ...
    for dim in dimList:
        ...
        assert scriptFolder.joinpath(
            f"./results/{D}dim/{exp_name}_best.pth.tar"
        ).exists(), "The model does not exist. Consider training the model by setting `train_model` to be true."
        # evaluation
